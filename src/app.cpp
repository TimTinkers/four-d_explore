...
void App::get_buffer_memory_offsets(uint32_t  n_sine_pair,
	...
	if (out_opt_sine1SB_offset_ptr != nullptr) {
		*out_opt_sine1SB_offset_ptr = static_cast<uint32_t>(outputDataOffsetSizes[n_sine_pair * 2]);
	}

	if (out_opt_sine2SB_offset_ptr != nullptr) {
		*out_opt_sine2SB_offset_ptr = static_cast<uint32_t>(outputDataOffsetSizes[n_sine_pair * 2 + 1]);
	}
...
void App::init_buffers() {
	...
	std::vector<glm::vec4> vertexData = { glm::vec4(), glm::vec4(), glm::vec4(), glm::vec4(),
	...
	for (unsigned int n_sine_pair = 0; n_sine_pair < N_SINE_PAIRS; ++n_sine_pair) {
		for (unsigned int n_sine = 0; n_sine < 2; ++n_sine) {
			...
			// Store current offset and account for space necessary to hold the generated sine data.
			outputDataOffsetSizes.push_back(outputVerticesBufferSize);
	...
	// Assign memory blocks to wave color buffer and fill with color values.
...
void App::init_command_buffers() {
	...
	for (unsigned int n_current_swapchain_image = 0;
		...
		for (unsigned int n_sine_pair = 0; n_sine_pair < N_SINE_PAIRS; ++n_sine_pair) {
			...
			draw_cmd_buffer_ptr->record_push_constants(computePipelineLayoutPointer,
		...
		* that the data we have generated in the compute shader is actually visible to the draw call. */
...
void App::init_dsgs() {
	...
	printf("dsg1\n");
	computeShaderDescriptorGroupPointer->add_binding(2, /* n_set      */
		0, /* binding    */
		VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC,
		1, /* n_elements */
		VK_SHADER_STAGE_COMPUTE_BIT);
